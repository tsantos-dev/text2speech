...
async function testMindmapRoute() {
  ...
  try {
    console.log('Enviando requisição para http://localhost:3000/mindmap/' + userId);
    
    const response = await fetch('http://localhost:3000/mindmap/' + userId, {
      method: 'GET',
      headers: {
    ...
    console.log('Headers da resposta:', [...response.headers].map(h => `${h[0]}: ${h[1]}`).join('\n'));
